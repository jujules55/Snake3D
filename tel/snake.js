...
export default function Snake(x, y, z, camera, scene) {
    ...
    this.update = function () {
        ...
        if ((posT.x - posP.x) * (posT.x - posP.x) + (posT.y - posP.y) * (posT.y - posP.y) + (posT.z - posP.z) * (posT.z - posP.z) < 0.99) {
            ...
            if (posT.x < -6.1 | posT.x > 6.1 | posT.y < -6.1 | posT.y > 6.1 | posT.z < -6.1 | posT.z > 6.1) {
                
                document.body.removeChild(document.getElementById('c'));
                document.body.removeChild(document.getElementById('fleches'));
                document.getElementById('score').innerHTML = this.corps.length - 2;
                document.getElementById('perdu').classList.replace('invisible', 'visible');

            } else {


                for (let i = 1; i < this.corps.length; i++) {
                    let pos = this.corps[i].position
                    if ((posT.x - pos.x) * (posT.x - pos.x) + (posT.y - pos.y) * (posT.y - pos.y) + (posT.z - pos.z) * (posT.z - pos.z) < 0.99) {
